...
def read(handle):
    ...
    while True:
        ...
        motif.regex = __read_regex(handle)
        __skip_unused_lines2(handle)
        record.append(motif)
...
def __read_regex(handle):
    for line in handle:
        if 'regular expression' in line:
            break
    else:
        raise ValueError('Unexpected end of stream: Failed to find regex')
    for line in handle:
        if line.startswith('---'):
            break
    else:
...
def __skip_unused_lines2(handle):
    for line in handle:
