	   ...
	   TR({-class=>'resultsbody'},
		 ...
		 pre(
		     textarea(-name=>'text',-value=>$example,
			      -cols=>110,-rows=>10,-wrap=>'off',-override=>length $example || param('Clear'))
		    )
		)
	     )
	  ),
...
sub render {
  ...
  my %all_types = map {$_=>1} $data->types;
  
  my @configured_types   = grep {exists $all_types{$_}}        $data->configured_types;
